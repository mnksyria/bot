...
def api_auth_login(payload: LoginRequest):
    """
    تسجيل الدخول/التسجيل بدون PIN - بس device_id + username.
    - جهاز جديد: بينشئ حساب ويرجع توكن.
    - جهاز موجود بنفس الاسم: بيرجع توكن لنفس الحساب.
    - جهاز موجود باسم مختلف: 403 - هاد الجهاز مسجل لحساب تاني.
    """
    try:
        result = auth_store.login_or_register(payload.device_id, payload.username)
    ...
    if result.get("error") == "username_mismatch":
        raise HTTPException(
            status_code=403,
            detail="هذا الجهاز مسجّل مسبقاً باسم مختلف. سجّل بنفس الاسم الأول أو تواصل مع الدعم.",
        )
    return result
...
def download_package(package_id: str, authorization: Optional[str] = Header(None)):
    ...
    # نقرأ رابط GitHub من packages.json (نفس الملف يلي بيرجعه /api/packages)
    manifest_path = PACKAGES_DIR / "packages.json"
    download_url = None
    if manifest_path.exists():
        with open(manifest_path, "r", encoding="utf-8") as f:
            manifest = json.load(f)
        entry = next((p for p in manifest if p.get("id") == package_id), None)
        if entry:
            download_url = entry.get("download_url") or None

    if download_url:
        # الملف مستضاف على GitHub - نحوّل مباشرة، Render ما بيلمس البايتات.
        return RedirectResponse(url=download_url, status_code=307)

    # احتياطي مؤقت: لو لسا ما انتقلنا لملف هاد الباكج على GitHub، منخدمه
    # محلياً من مجلد packages/ (نفس السلوك القديم).
